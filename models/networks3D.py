...
# ResViT
from . import residual_transformers as residual_transformers
...
def define_G(input_nc, output_nc, ngf, netG, img_size, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    net = None
    ...
    if netG == 'resnet_9blocks':
        ...
        net = residual_transformers.ResViT(residual_transformers.CONFIGS['Res-ViT-B_16'],
                                           input_dim=input_nc, img_size=img_size,
...
# Defines SSIM Loss to be used with Cycle Consistency if Desired (From MONAI)
# https://docs.monai.io/en/1.1.0/_modules/monai/losses/ssim_loss.html
class SSIMLoss(_Loss):
    def __init__(self, win_size: int = 7, k1: float = 0.01, k2: float = 0.03, spatial_dims: int = 3):
        """
        Args:
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, data_range: torch.Tensor) -> torch.Tensor:
        ...
        """
        if x.shape[0] == 1:
            ssim_value: torch.Tensor = SSIMMetric(spatial_dims=self.spatial_dims,
                data_range=data_range, win_size=self.win_size, k1=self.k1, k2=self.k2
            )._compute_tensor(x, y)
        elif x.shape[0] > 1:

            for i in range(x.shape[0]):
                ssim_val: torch.Tensor = SSIMMetric(spatial_dims=self.spatial_dims,
                    data_range=data_range, win_size=self.win_size, k1=self.k1, k2=self.k2
...
# Defines the Unet generator. |num_downs|: number of downsamplings in UNet
class UnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64,
        ...
        # Set Up for Sequential Gradient Checkpointing
        modules = [module for k,module in self.model._modules.items()]
        self.modules = [module for k,module in modules[0]._modules.items()]

    def forward(self, input):
        return checkpoint_sequential(self.modules, len(self.modules), input) #self.model(input)
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        if outermost:
            ...
            upconv = nn.ConvTranspose3d(inner_nc, outer_nc,
                                        kernel_size=4, stride=2,
                                        padding=1, bias=use_bias)
            convup = nn.Conv3d(inner_nc, inner_nc, kernel_size=3,
                               stride=1, padding=1, bias=use_bias)
